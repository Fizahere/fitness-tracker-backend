...
export const uploadImageToGitHub = async (filePath, fileName) => {
    try {
        ...
        const headers = { 
            Authorization: `Bearer ${process.env.GITHUB_TOKEN}`,  // Required for uploads
        ...
        if (sha) {
            uploadData.sha = sha; // Required if updating an existing file
        }

        const response = await axios.put(githubUrl, uploadData, { headers });
